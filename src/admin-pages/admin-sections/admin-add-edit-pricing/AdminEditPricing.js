...
function AdminEditPricing() {
    ...
    useEffect(() => {
        if (cookies.userToken) {
            ...
            verifyJWT()
            setPricingID(searchParams.get('pricing_id'))
            setLoading(true)
            ...
            .then(data => {
                if(data.pricings){
                    data.pricings.map(pricing => {
                        if (pricing._id == searchParams.get('pricing_id')) {
                            setClassification(pricing.classification)
                            setDescription(pricing.description)
                            setPrice(pricing.price)
                            setClasses(pricing.classes)
                        } else {
    ...
    const handleSubmit = (e)=>{
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({pricingID: pricingID,description: description, classification: classification, price: price, classes: classes})
        };
        fetch(`${process.env.REACT_APP_BACKEND_URL}/edit-pricing`, requestOptions)
            .then(response => response.json())
            .then(data => {
                if (data.msg) {
                    toast.info(data.msg)
