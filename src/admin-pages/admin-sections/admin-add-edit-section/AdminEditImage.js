...
function AdminEditImage() {
    ...
    const [homeSectionImage, setHomeSectionImage] = useState("")
    ...
    useEffect(() => {
        if (cookies.userToken) {
            ...
            setImageID(searchParams.get('image_id'))
            fetch(`${process.env.REACT_APP_BACKEND_URL}/images/section-images/${searchParams.get('sec_id')}`)
                .then(response => response.json())
                .then(data => {
                    data.pictures.map(img => {
                        if (img._id === searchParams.get('image_id')) {
                            setSectionImageID(img._id)
                            setSectionImageLink(img.link)
                        }
                    })
                    setLoading(false)
            }).catch(error =>{
                        ...
                        <div className="products-area-wrapper gridView" style={{justifyContent: 'center'}}>
                                <div className="products-row no-pointer" style={{width: "calc(50% - 16px)"}}>
                                    ...
                                    <div className="center">
                                        ...
                                        <div className="dropzone" onClick={changeSectionImage}>
